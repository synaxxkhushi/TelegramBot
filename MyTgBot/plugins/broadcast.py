import asyncio
from MyTgBot.database.users_db import *
from MyTgBot import bot
from pyrogram import filters

@bot.on_message(filters.command("pcast"))
async def broadcast(_, message):
    ...
    for chat in chats:
      try:
        await bot.forward_messages(chat_id=int(chat), from_chat_id=message.chat.id, message_ids=to_send)
    ...
    for user in users:
      try:
        await bot.forward_messages(chat_id=int(user), from_chat_id=message.chat.id, message_ids=to_send)
        await asyncio.sleep(1)
